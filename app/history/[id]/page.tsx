...
import { buildGraph } from "@/core/analysisPipeline";
...
export default function HistoryDetailsPage() {
  ...
  const graphData = analysis?.graph || analysis?.report?.graph || (
    analysis?.smiles && patents?.length > 0
      ? buildGraph(
          analysis.smiles,
          analysis.target,
          analysis.disease,
          patents.map((p: any) => ({
            patentNumber: p.patent_number || p.patentNumber,
            title: p.patent_title || p.title,
            publicationDate: p.publication_date || p.publicationDate,
            assignee: p.assignee,
            abstract: p.abstract,
            source: p.source,
            relevanceScore: Number(p.relevance_score || p.relevanceScore || 0),
            confidence: Number(p.confidence_score || p.confidence || 0),
            aiExplanation: p.ai_explanation || p.aiExplanation,
          }))
        )
      : null
  );
          ...
          <div className="p-6 sm:p-10">
            ...
            {patents.length > 0 && (
              <div id="related-patents" className="mt-10">
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200">
                  <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 bg-slate-50 px-6 py-4 sm:px-8">
                    ...
                    <button
                      onClick={() => {
                        document.getElementById("interactive-graph")?.scrollIntoView({ behavior: "smooth" });
                      }}
                      className="inline-flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-bold text-white bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 transition-all shadow-sm hover:scale-105"
                    >
            ...
            {graphData && graphData.nodes?.length > 0 && (
              <div id="interactive-graph" className="mt-10">
                <p className="font-sans mb-3 text-[11px] font-bold uppercase tracking-widest text-indigo-600/70">
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200">
                  <div className="flex items-center justify-between gap-2 border-b border-slate-200 bg-slate-50 px-6 py-4 sm:px-8">
                    <h2 className="text-xl font-bold text-slate-900">Innovation Relationship Map</h2>
                    <span className="font-sans rounded-full bg-white border border-slate-200 px-3 py-1 text-xs font-semibold text-slate-500">
                      Interactive
                    </span>
                  </div>

                  <PatentWorkflowGraph graph={graphData} />
                </div>
              </div>
            )}
