...
export async function generateReport(
  ...
  const avgRelevance =
    patents.reduce((sum, p) => sum + (p.relevanceScore || 0), 0) /
    Math.max(1, patents.length);

  const avgConfidence =
    patents.reduce((sum, p) => sum + (p.confidence || 0), 0) /
    Math.max(1, patents.length);

  // Overall patentability score
  const calculatedScore = Math.round(
    avgRelevance * 0.7 + avgConfidence * 0.3
  );
...
Explain why the retrieved patents support this recommendation.

The field "scoringMethodology" should briefly explain how the score was determined.
...
`;
  ...
  let report: PatentabilityReport;

  try {
    report = JSON.parse(cleaned);
  } catch (error) {
    console.error("========== FEATHERLESS RAW RESPONSE ==========");
    console.error(text);
    console.error("==============================================");
    throw new Error("Featherless returned invalid JSON.");
  }

  // Enforce application-computed values
  report.overallScore = calculatedScore;
  report.recommendation = recommendation as any;
  report.overallRisk = overallRisk as any;
  ...
  if (!allowedRecommendations.includes(report.recommendation)) {
    report.recommendation = "Requires Expert Review" as any;
  }

  const allowedRisk = ["Low", "Medium", "High"];

  if (!allowedRisk.includes(report.overallRisk)) {
    report.overallRisk = "Medium" as any;
  }
  ...
  report.noveltyConcerns ??=
    "Insufficient metadata to identify specific novelty concerns.";
  report.manualReviewPatents ??= [];
  report.scoringMethodology ??=
    "The overall patentability score is determined by considering the relevance scores of the retrieved patents, AI-assisted semantic analysis, chemical overlap with the submitted compound, therapeutic overlap with the intended target or disease, confidence scores, and identified novelty concerns.";
  report.justification ??= "";
